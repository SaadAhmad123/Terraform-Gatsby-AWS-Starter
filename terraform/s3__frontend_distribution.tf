...
resource "aws_s3_bucket" "frontend_distribution" {
  bucket        = "${local.name_prefix}-frontend-dist"
  tags          = local.tags
}

resource "aws_s3_object" "frontend_distribution" {
  for_each = fileset("${path.module}/../gatsby_frontend/public", "**/*")
  bucket   = aws_s3_bucket.frontend_distribution.id
  key      = "${each.value}"
  source   = "${path.module}/../gatsby_frontend/public/${each.value}"
  etag     = filemd5("${path.module}/../gatsby_frontend/public/${each.value}")
...
resource "null_resource" "frontend_cloudfront_invalidation" {
  ...
  provisioner "local-exec" {
    command = "AWS_ACCESS_KEY_ID=${var.AWS_ACCESS_KEY} AWS_SECRET_ACCESS_KEY=${var.AWS_SECRET_KEY} aws cloudfront create-invalidation --distribution-id ${aws_cloudfront_distribution.frontend_distribution.id} --paths '/*'"
  }
